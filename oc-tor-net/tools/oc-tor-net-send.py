...
Requires the P2P daemon (oc-tor-net-start.py) to be running for Tor connectivity.
"""
import sys
import json
import os
...
def main(peer_address: str, message_text: str):
    ...
    # Check daemon is running (needed for Tor SOCKS proxy)
    daemon_status = TorManager.check_daemon(str(config_dir))
    if not daemon_status['running']:
        print(f"Error: {daemon_status['error']}")
        print("The daemon must be running to send messages (provides Tor connectivity).")
        sys.exit(1)
